...
class CitrusDataset(Dataset):
    ...
    def __init__(self, root_dir='./', type_="train", size=None, transform=None):
        ...
        if self.type != 'test':
            instance_list_all = glob.glob(os.path.join(self.root_dir, 'images/{}/'.format(self.type), '*_label.png'))
    ...
    def __getitem__(self, index):
        ...
        if self.type != 'test':
            ...
            instance_map_all = cv2.resize(instance_map_all, (512, 512), interpolation=cv2.INTER_NEAREST)
            ...
            for class_id in class_ids:
                ...
                elif (class_id != 255) and (class_id != 148):
                    diff_plant = 255 - class_id
                    diff_bg = class_id - 0

                    if diff_plant > diff_bg:
                        mask_bg = (instance_map_all == class_id)

                        instance_all[mask_bg] = 0
                        label_all[mask_bg] = 0
                    else:
                        mask_plant2 = (instance_map_all == class_id)

                        instance_all[mask_plant2] = 1
                        label_all[mask_plant2] = 1
